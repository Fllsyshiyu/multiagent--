...
export function createScriptedCaller(script: ScriptData): LLMCaller {
  ...
  return async (system: string, user: string): Promise<{ text: string; tokens: number }> => {
    ...
    if (ww) {
      // GenericGameEngine 使用结构化 GAME_CONTEXT；优先按 action_id 匹配，
      // 保持旧的六人预录剧本与新的通用运行时兼容。
      if (actionId === 'wolf_talk') return json(ww.wolf_talk[agentId] ?? { content: '先观察发言再统一目标。', suggest_target: 'p3' }, 320)
      if (actionId === 'wolf_kill') {
        const target = Object.values(ww.wolf_talk).at(-1)?.suggest_target ?? 'p3'
        return json({ target }, 260)
      }
