 ...
 * 复用通用策略：A5(私聊) B5(角色权限信息) A3(全体发言) E7(投票决议)
 * 扩展独有：淘汰、秘密角色分派、胜负判定
...
const ROLE_INFO: Record<string, { team: string; desc: string }> = {
  werewolf: { team: '狼人阵营', desc: '夜晚与同伴私聊（A5）选择袭击目标；白天隐藏身份' },
  seer: { team: '好人阵营', desc: '每晚查验一名玩家身份（B5：结果仅自己可见）' },
  witch: { team: '好人阵营', desc: '拥有一瓶解药和一瓶毒药（B5：仅自己知道用药情况）' },
...
export class WerewolfGame {
  ...
  async run(userInput: string): Promise<void> {
    ...
    this.emit({
      t: 'phase_start', phase_id: 'setup', name: '秘密角色分派', purpose: '扩展独有逻辑：身份仅本人可见（B5 权限信息）',
      strategy: { A: ['A3'], B: 'B5', C: 'C1', D: 'D1', E: ['E1'], notes: ['复用 B5：每个玩家只能看到自己的身份牌'] },
    })
    ...
    this.emit({
      t: 'phase_start', phase_id: 'night_1', name: '第 1 夜 · 狼人私聊', purpose: 'A5 私下沟通：内容不进入公共工件（观众以上帝视角可见）',
      strategy: { A: ['A5'], B: 'B5', C: 'C1', D: 'D1', E: ['E1'], notes: ['A5 私聊通道：仅狼人阵营'] },
    })
...
现在是夜晚，你在狼人私聊频道（A5）与同伴商议今晚袭击谁。其他玩家：${players.filter((p) => p.secret_role !== 'werewolf').map((p) => `${p.name}(${p.id})`).join('、')}。
${wolfPlan.length ? `同伴刚才说：${wolfPlan[0].content}` : '你先发言。'}`,
    ...
    const seer = players.find((p) => p.secret_role === 'seer')!
    {
      ...
      this.emit({ t: 'game_event', event: { kind: 'WerewolfAction', round: 1, actor: seer.id, action: 'check', target: target.id, result: `预言家查验 ${target.name} → ${isWolf ? '🐺 狼人' : '好人'}（B5：仅预言家可见）`, visible_to: [seer.id, 'god'] } })
    ...
    let poisonVictim: Player | null = null
    {
      ...
      this.emit({ t: 'game_event', event: { kind: 'WerewolfAction', round: 1, actor: witch.id, action: saved ? 'save' : 'poison', target: saved ? victim.id : (poisonVictim?.id ?? undefined), result: saved ? `女巫使用解药救下 ${victim.name}（B5：仅女巫可见）` : `女巫选择不救${poisonVictim ? `，并对 ${poisonVictim.name} 使用毒药` : ''}（B5：仅女巫可见）`, visible_to: [witch.id, 'god'] } })
      await this.paced(280)
    }
    ...
    this.emit({
      t: 'phase_start', phase_id: 'day_1', name: '第 1 天 · 全体发言', purpose: 'A3 全体参与 + B5：每人只掌握自己的信息',
      strategy: { A: ['A3'], B: 'B5', C: 'C1', D: 'D1', E: ['E1'], notes: ['A3 按顺序全体发言'] },
    })
...
${p.secret_role === 'werewolf' ? '你是狼人，要误导好人、嫁祸他人，但不能暴露自己和同伴（同伴是 ' + wolves.filter((w) => w.id !== p.id).map((w) => w.name).join('') + '）。' : ''}`,
    ...
    this.emit({
      t: 'phase_start', phase_id: 'vote_1', name: '投票表决', purpose: 'E7 投票决议：收集投票 → 执行结果',
      strategy: { A: ['A3'], B: 'B5', C: 'C1', D: 'D2', E: ['E7'], notes: ['E7：得票最多者出局'] },
    })
...
${p.secret_role === 'seer' ? `你掌握：${transcript.join('；')}。` : ''}`,
    ...
    const markdown = [
      ...
      ``,
      `### 本局复用的通用策略（核心框架代码零改动）`,
      `- **A5 私下沟通**：狼人夜间密谋，内容不进入公共工件`,
      `- **B5 角色权限信息**：身份牌、预言家查验、女巫用药均按权限路由`,
      `- **A3 全体参与**：白天按序发言`,
      `- **E7 投票决议**：收集投票并执行淘汰`,
      ``,
      `### 扩展独有逻辑（不进核心框架）`,
